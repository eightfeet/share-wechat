...
function isWeChat() {
  ...
  var compare = ua.match(/MicroMessenger/i) || '';
...
function loaded(url) {
  if (!url) {
    console.error('请引入JS文件！！！！！！');
    return;
  }
  return new Promise(function (resolve, reject) {
    if (window.wx && _typeof(window.wx) === 'object') {
      wx = window.wx;
      resolve();
      return;
    }
...
function configs(config) {
  return new Promise(function (resolve, reject) {
    ...
    var shareArr = ['onMenuShareTimeline', 'onMenuShareAppMessage', 'onMenuShareQQ', 'onMenuShareWeibo', 'onMenuShareQZone'];
    ...
    var debug = getConfig.debug,
        ...
        nonceStr = getConfig.nonceStr,
        signature = getConfig.signature,
        jsApiList = getConfig.jsApiList;
...
function share(data, success, cancel) {
  // window.alert(1);
  //  = arguments[0]
  var Info = {};
...
function wechat(url, config) {
  return Promise.resolve().then(function () {
    return loaded(url);
  }).then(function (res) {
    return configs(config).then(function () {
      return res;
    });
  });
}
